...
def angle(*vec):
    v = magnitude(*vec)
    return [np.rad2deg(np.arccos(x/v)) for x in vec]
...
def unit(i,j,k):
    v = magnitude(*vec)
    return str(i/v)+'i '+ str(j/v) + 'j '+ str(k/v)+ 'k '
print(unit(2,-6,-3))
...
def angleoftwo(i1, j1, k1, i2, j2, k2):
    angle = np.rad2deg(np.arccos(dot(i1, j1, k1, i2, j2, k2)/(magnitude(i1, j1, k1)*magnitude(i2, j2, k2))))
    return angle

#projection of a vector
def projection(i1, j1, k1, i2, j2, k2):
    proj_ab = dot(i1, j1, k1, i2, j2, k2)/magnitude(i2, j2, k2) #this is the projection of a onto b
    proj_ba = dot(i2, j2, k2, i1, j1, k1)/magnitude(i1, j1, k1) #this is the projection of b onto a
...
def angleofR(magnitude1, magnitude2, alpha):
    alpha = np.deg2rad(alpha)
    theta = np.rad2deg(np.arctan((magnitude2*np.sin(alpha))/(magnitude1 + magnitude2*np.cos(alpha))))
    return theta
